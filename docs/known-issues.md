...
## BUG-005: JsonConverter のラウンドトリップ精度劣化 📝 対応不要（制限事項として記載）
...
| 対応 | コード修正なし。APIマニュアルに制限事項として記載する。JSON経由のラウンドトリップで±1%程度の誤差が生じる場合がある。精度が必要な場合はMIDI文字列を直接使用すること |
...
## BUG-006: play() の戻り値が空データ時に不統一 ✅ 修正済み（2026-04-25）
...
| 対応 | `get_code()` の falsy 入力時の戻り値を `undefined` から空配列 `[]` に統一 |
...
| BUG-005 | 中 | あり | JsonConverter のラウンドトリップ精度劣化 | 📝 制限事項 |
| BUG-006 | 低 | あり | play() の戻り値が空データ時に不統一 | ✅ 修正済み |
